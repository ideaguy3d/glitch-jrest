...
var routes = function(app) {
  ...
  // this route is simply for practice 
  app.get("/simple", function(req, res) {
    let check = req.query.check; 
    let checkStatus =  "x-rsm-check-status";
    let jsonResponse = {}; 
    
    if(check) {
        if(check === "red0101f") {
           jsonResponse[checkStatus] = "Request successfully passed the check";
           res.send(jsonResponse);    
        }
        else {
          jsonResponse[checkStatus] = "Request failed check";
          res.send(jsonResponse);
        } 
    }
    else {
      jsonResponse[checkStatus] = "Request had no checks";
      res.send(jsonResponse); 
    }
  }); 
